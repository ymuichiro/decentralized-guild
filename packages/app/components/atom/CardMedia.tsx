...
export default function CardMedia(props: P): JSX.Element {
  const theme = useTheme();

  const handleClick = () => {
    props.onClick(props.quest);
  };

  return (
    <ButtonBase style={{ width: '100%', height: '100%' }} onClick={handleClick}>
      <Card
        elevation={0}
        sx={{
          display: 'flex',
      ...
      >
        <MuiCardMedia sx={{ height: 160 }} image={DEFAULT_IMAGE} title='quest image' />
        <CardContent style={{ paddingBottom: '0px' }}>
          <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
            <Chip
              ...
              size='small'
              color={
                props.quest.status === 'WANTED' ? 'success' : props.quest.status === 'WORKING' ? 'secondary' : 'default'
              }
            />
          </div>
          <Typography
        ...
        >
          <Typography variant='body2' textAlign={'left'}>
            {TextService.trimText(TextService.mdToText(props.quest.description), 100)}
          </Typography>
        </CardContent>
        <CardContent style={{ textAlign: 'left' }}>
          <Typography variant='body2'>{props.quest.createdAt.toLocaleDateString()}</Typography>
        </CardContent>
      </Card>
    </ButtonBase>
  );
